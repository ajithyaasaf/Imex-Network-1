...
// ============================================================================
// Types (inlined to avoid any external imports)
// ============================================================================
...
// ============================================================================
// Chatbot Engine (inlined)
// ============================================================================
const conversationHistory = new Map<string, ChatMessage[]>();

function findBestMatch(message: string): FAQItem | null {
        ...
        .map(faq => {
            ...
            faq.keywords.forEach(keyword => {
                if (lowerMessage.includes(keyword.toLowerCase())) {
                    if (lowerMessage === keyword.toLowerCase()) score += 10;
                    else if (lowerMessage.startsWith(keyword.toLowerCase())) score += 5;
                    else score += 1;
                }
            });
            return { faq, score };
...
function generateResponse(message: string, conversationId?: string): ChatMessage {
    ...
    if (match) {
        ...
        if (conversationId) addToHistory(conversationId, response);
        return response;
    }
    ...
    if (conversationId) addToHistory(conversationId, fallback);
    return fallback;
}
...
function addToHistory(conversationId: string, message: ChatMessage): void {
    ...
    conversationHistory.get(conversationId)!.push(message);
}
...
export default async function handler(req: VercelRequest, res: VercelResponse) {
    ...
    try {
        ...
        if (req.method === 'POST' && url.includes('/chat/message')) {
            ...
            addToHistory(convId, userMessage);

            const typingDelay = 300 + Math.random() * 500;
            await new Promise((resolve) => setTimeout(resolve, typingDelay));

            const botResponse = generateResponse(message, convId);
        ...
        if (req.method === 'GET' && url.includes('/chat/history/')) {
            ...
            return res.status(200).json({ messages: getHistory(cid) });
        }
